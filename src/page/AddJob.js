...
import { createNewJob } from '../features/jobData/jobDataSlice';
import nextId from '../utils/nextId';

const AddJob = () => {
    const dispatch = useDispatch();
    // const navigate = useNavigate();
    const { jobData: jobList } = useSelector(state => state.jobData);
    ...
    const handleAddJobData = (event) => {
        ...
        const isEmpty = Object.values(jobInfo).some(x => x === null || x === '');
        if (!isEmpty) {
            const id = nextId(jobList);
            dispatch(createNewJob({ ...jobInfo, id }));
            // navigate("/");
            setJobInfo({
                title: "",
                type: "",
                ...
                <div className="max-w-3xl mx-auto">
                    <form className="space-y-6">
                        ...
                        <div className="text-right">
                            <button type="submit" id="lws-submit" className="cursor-pointer btn btn-primary w-fit" onClick={handleAddJobData}>
                                Add
                            </button>
                        </div>
                    </form>
